...
import { canLaunchDirectly, launchGame } from './launcher'
...
describe('launchGame', () => {
  it('spawns the derived executable with the Doorstop arguments', () => {
    ...
    expect(launchGame(root, plan, 'win32', spawn).started).toBe(true)
  ...
  it('explains itself instead of failing on macOS', () => {
    const { spawn } = fakeSpawn()
    const outcome = launchGame(root, buildLaunchPlan('/p'), 'darwin', spawn)
  ...
  it('refuses a folder that is not a Unity build', () => {
    ...
    const outcome = launchGame(empty, buildLaunchPlan('/p'), 'win32', fakeSpawn().spawn)
    expect(outcome.started).toBe(false)
  ...
  it('reports a missing executable rather than spawning nothing', () => {
    ...
    const outcome = launchGame(noExe, buildLaunchPlan('/p'), 'win32', fakeSpawn().spawn)
    expect(outcome.started).toBe(false)
