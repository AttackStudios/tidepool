...
developed and tested against existing communities ahead of release. There's a community Discord at
{{DISCORD_INVITE}} with modders lined up.

The game ships with a built-in level editor, so custom breaks are an obvious content category on top
of code mods — we expect steady package volume rather than a handful of uploads.
