...
**Host:** load a beach, press **F9**. The log prints your Steam ID and writes it
to `UserData/surfmp-my-id.txt`. Send that number to the testers.

**Testers:** put that number in `UserData/surfmp-join.txt`, load the same beach,
press **F10**.
...
- **Steam invites are not wired up** — testers paste an ID rather than clicking a
  friend. The relay is what mattered; the invite flow is cosmetic by comparison.
